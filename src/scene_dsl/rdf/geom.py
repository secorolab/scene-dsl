...
def add_position_coord(
    ...
    add_vector_xyz(graph=graph, node=pos_coord_uri, vect=xyz_vector)
...
def add_pose(graph: Graph, pose: PoseSpec) -> None:
    ...
    graph.add(triple=(pose.uri, URI_QUDT_PRED_QUANTITY_KIND, URI_QUDT_QK_ANGLE))
    if isinstance(pose.orientation, EulerOrientationSpec):
        graph.add((pose.uri_coord, RDF.type, URI_GEOM_TYPE_EULER_ANGLES))
        graph.add((pose.uri_coord, URI_GEOM_PRED_AXES_SEQ, Literal(pose.orientation.axes)))
        graph.add(
            (
                pose.uri_coord,
                RDF.type,
        ...
        graph.add((pose.uri_coord, RDF.type, URI_GEOM_TYPE_ANGLES_ABG))
        graph.add((pose.uri_coord, URI_GEOM_PRED_ALPHA, Literal(pose.orientation.alpha)))
        graph.add((pose.uri_coord, URI_GEOM_PRED_BETA, Literal(pose.orientation.beta)))
        graph.add((pose.uri_coord, URI_GEOM_PRED_GAMMA, Literal(pose.orientation.gamma)))
        graph.add(
            (
                pose.uri_coord,
                URI_QUDT_PRED_UNIT,
                URI_QUDT_UNIT_DEG if pose.orientation.unit == "deg" else URI_QUDT_UNIT_RAD,
            )
        )
    elif isinstance(pose.orientation, DirectionCosineOrientationSpec):
        graph.add((pose.uri_coord, RDF.type, URI_GEOM_TYPE_DIRECTION_COSINE_XYZ))
        add_literal_list_pred(
            graph=graph,
            subject_uri=pose.uri_coord,
