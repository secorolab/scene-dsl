...
def add_pose(graph: Graph, pose: PoseSpec) -> None:
    ...
    graph.add((pose.uri, RDF.type, URI_GEOM_TYPE_POSITION_REF))
    graph.add((pose.uri, URI_GEOM_PRED_OF_POSITION, pose.position_uri))
    graph.add((pose.uri, RDF.type, URI_GEOM_TYPE_ORIENT_REF))
    graph.add((pose.uri, URI_GEOM_PRED_OF_ORIENT, pose.orientation_uri))
